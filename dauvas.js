...
function triangleRotate(x1, y1, x2, y2, x3, y3, alpha) {
    //rotiert um den Schwerpunkt
    var xs = (x1 + x2 + x3) / 3;
    var ys = (y1 + y2 + y3) / 3;
    var x1s = x1 - xs,
        x2s = x2 - xs,
        x3s = x3 - xs; //Koordinaten bzg. Schwerpunkt
    var y1s = y1 - ys,
        y2s = y2 - ys,
        y3s = y3 - ys; //Koordinaten bzg. Schwerpunkt
    ...
    var x1rot = x1srot + xs,
        x2rot = x2srot + xs,
        x3rot = x3srot + xs; //Koordinaten der neuen Punkte
    var y1rot = y1srot + ys,
        y2rot = y2srot + ys,
        y3rot = y3srot + ys;
    ctx.beginPath();
...
function fillTriangleRotate(x1, y1, x2, y2, x3, y3, alpha) {
    //rotiert um den Schwerpunkt
    var xs = (x1 + x2 + x3) / 3;
    var ys = (y1 + y2 + y3) / 3;
    var x1s = x1 - xs,
...
function fillRectCenter(x, y, width, height) {
    // Mitte, Breite, Höhe
...
function fillPoly(flatArray) {
    // e.g. [0,0,100,100,50,0]
    // copy array
